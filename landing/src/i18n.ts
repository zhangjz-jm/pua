...
export const BENCHMARKS = [
  ...
  },
  {
    ...
    desc: i("Learning-driven. Best for fixed thinking: every failure must produce a changed hypothesis, changed action, and verification evidence.", "学习驱动。适合思维固化：每次失败都必须产出改变后的假设、行动和验证证据。", "学習駆動。固定思考に最適：失敗ごとに仮説・行動・検証証拠を変える。", "Piloté par l'apprentissage. Idéal contre la pensée fixe : chaque échec doit changer l'hypothèse, l'action et la preuve.", "Lerngetrieben. Ideal bei fixiertem Denken: Jeder Fehler muss Hypothese, Aktion und Beleg ändern.", "مدفوع بالتعلم. الأفضل للتفكير الثابت: كل فشل يجب أن يغير الفرضية والعمل ودليل التحقق."),
    metrics: { fix_depth: 84, reasoning_structure: 88, verification_rigor: 86, root_cause_analysis: 82 },
    sample: i("Growth Mindset is not a slogan. What did the failure teach you, and how does it change the next action?", "Growth Mindset 不是口号。失败教会了什么？下一步动作为什么不同？", "Growth Mindset はスローガンではない。失敗から何を学び、次の行動はどう変わる？", "Growth Mindset n'est pas un slogan. Qu'as-tu appris et comment l'action change ?", "Growth Mindset ist kein Slogan. Was hast du gelernt und wie ändert sich die nächste Aktion?", "عقلية النمو ليست شعاراً. ماذا علمك الفشل وكيف سيغيّر الإجراء التالي؟"),
  },
]
